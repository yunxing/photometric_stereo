...
c
    ...
    channel = img_np.shape[3]

    lights_t = np.transpose(lights_np)
    N = np.tensordot(
        np.linalg.inv(lights_t @ lights_np) @ lights_t, img_np, axes=([1], [0])
    )
    N = np.transpose(N, [1, 2, 0, 3])

    # print(intensity.shape)
    # print(N.shape)
    N_no_channel = np.sum(N, axis=-1) / channel
    n = N_no_channel / np.linalg.norm(N_no_channel, axis=-1, keepdims=True)
    # albedo calculation
    albedo = np.squeeze(np.linalg.norm(N, axis=2, keepdims=True), axis=2)
